  ...
  return (
    <div>
      <div className="dashboard-title-row">
        ...
        <Link href="/admin/motorcycles/new" className="button-primary dashboard-add-button"><CirclePlus size={19} /> Add motorcycle</Link>
      </div>
      ...
      {sales.recentSales.length > 0 ? <section className="mt-9"><div className="flex items-end justify-between"><div><p className="eyebrow">Sales</p><h2 className="mt-1 text-xl font-black">Recent sales</h2></div><Link href="/admin/sales" className="text-sm font-bold text-[var(--primary)]">View all</Link></div><div className="dashboard-sales card mt-4">{sales.recentSales.map(({ sale, motorcycle, customer }) => <Link href={`/admin/motorcycles/${motorcycle.id}`} key={sale.id}><div><strong>{displayMotorcycleName(motorcycle.brand, motorcycle.model)}</strong><small>{customer?.name ?? "Buyer"} · {sale.soldAt.toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</small></div><b>{formatPrice(sale.sellingPrice, sale.currency)}</b></Link>)}</div></section> : null}
      <section className="mt-9">
        <div className="flex items-end justify-between"><div><p className="eyebrow">Inventory</p><h2 className="mt-1 text-xl font-black">Recently added</h2></div><Link href="/admin/motorcycles" className="text-sm font-bold text-[var(--primary)]">View all</Link></div>
