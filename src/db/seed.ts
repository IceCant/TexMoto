...
async function seed() {
  const [business] = await db
    ...
    .onConflictDoUpdate({ target: businesses.slug, set: { name: "TexMoto", updatedAt: new Date() } })
    .returning();
  if (!business) throw new Error("Failed to seed TexMoto.");
  ...
  await db
    ...
    .onConflictDoUpdate({ target: users.email, set: { businessId: business.id, name: "TexMoto Owner", updatedAt: new Date() } });
  ...
  console.info("Seeded TexMoto. Login: owner@texmoto.test / TexMoto123!");
}
